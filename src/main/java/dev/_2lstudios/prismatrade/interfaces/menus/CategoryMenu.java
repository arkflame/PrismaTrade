...
public class CategoryMenu extends PageMenu {
    ...
    public CategoryMenu(int page, Configuration config, PrismaTradeAPI prismaTrade, CategoryItemType categoryItemType,
    ...
    public void onBuild(MenuBuildContext context) {
        ...
        context.setTitle(Placeholder.color(config.getString("messages.category-menu-title")
                .replace("%page%", String.valueOf(getPage())).replace("%type%", categoryItemType.name())));

        for (int i = 0; i < MAX_ENTRY_COUNT && i < dataFound.length; i++) {
            Category data = dataFound[i];
            InterfaceItem item = new CategoryEntryItem(config, data);
        ...
        if (page > 1) {
            context.setItem(45, new PageItem(config, new CategoryMenu(page - 1, config, prismaTrade, categoryItemType, lastMenu),
                    PageType.PREVIOUS));
        } else {
        ...
        if (dataFound.length > MAX_ENTRY_COUNT) {
            context.setItem(53, new PageItem(config, new CategoryMenu(page + 1, config, prismaTrade, categoryItemType, lastMenu),
                    PageType.NEXT));
        }
    }
}
